    ...
    const timelineData = [
        {
            ...
            title: "Big cleanup under the hood",
            text: `<p>Heyy!! Ok so I finally sat down and did the giant cleanup on the site that has been living rent free in my head for months. Nothing looks different on the outside (that's kinda the whole point) but under the hood it's a completely different story.</p>

            <p>I renamed something like 470 files and folders so everything is properly named now, no more weird spaces or random capital letters in paths. That alone saved me so many headaches with GitHub Pages since the server there cares about capitalization and stuff kept quietly breaking on me.</p>

            <p>I also made a new <b>tokens.css</b> file where I put all my variables (colors, blur, shadows, fonts, timings) so if I ever want to tweak the blue on the site I don't have to dig through 40 different files to find every place it shows up. And I made a <b>fonts.css</b> that loads all my fonts in one shot instead of the 11 times per page they were being loaded before. Yes, eleven times. I know.</p>

            <p>Also got rid of <b>jQuery</b> completely. It was still sitting there in two important files (the diary music player and the calendar planner) and I rewrote both of them in plain vanilla JavaScript. Ripped out every <code>onclick=</code> from the HTML while I was at it, they were scattered around like confetti and I swapped them all for proper event listeners.</p>

            <p>I made shared components for the nav bar, tooltip, the falling stars, and the little cursor hearts too, so every page now uses the same code. There used to be like 4 copies of the music player and the nav bar floating around, it was kinda a mess honestly.</p>

            <p>Some other little things: I fixed a couple of broken links that had been there forever (an arrow image, an ambient mp3, some fonts pointing to random paths), patched up about 10 pages that had malformed HTML with duplicate <code>&lt;html&gt;</code> and <code>&lt;body&gt;</code> tags, and cleaned out three jQuery imports that weren't even being used anywhere.</p>

            <p>Overall way more lines got deleted than added, which is usually a good sign when you're cleaning house. The site is a lot easier to maintain now and I get way less anxious opening the editor when I want to tweak something small :)</p>`
        },
        {
            id: 1,
            date: "August 10, 2025",
