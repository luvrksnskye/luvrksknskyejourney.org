    ...
    var b = {
        init: function(b) {
            ...
            var c = {
              ...
              , b = a.extend(c, b);
            0 === a("#s-m-t-tooltip").length && a("body").append("<div id='s-m-t-tooltip'><div></div></div>");
            var d = a("#s-m-t-tooltip");
            return d.css({
                position: "absolute",
                display: "none"
            }).data("smt-z-index", d.css("z-index")).children("div").css({
                width: "100%",
                height: "100%"
            }),
            a(document).on("mouseout mousedown click", ".smt-current-element", function() {
                var c = a(this);
                clearTimeout(smtTooltip_delay),
                d.style_my_tooltips("hide", {
                    speed: c.data("smt-fade-speed")
                }),
                a(document).unbind("mousemove"),
                c.removeClass("smt-current-element"),
                "" === c.attr(b.attribute) && c.attr(b.attribute, c.data("smt-title"))
            }),
            this.on("mouseover", function() {
                var f = a(this)
                  , g = f.attr(b.attribute);
                f.addClass("smt-current-element").data({
                    "smt-title": g,
                    "smt-fade-speed": b.tip_fade_speed
                }).attr(b.attribute, ""),
        ...
        update: function(b) {
            ...
            c.stop().css({
                display: "none",
                "z-index": c.data("smt-z-index")
            }).children("div").text(b.title),
