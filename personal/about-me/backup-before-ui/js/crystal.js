...
export class CrystalField {
  constructor({ stage, section, panel, intro, onBuild, onHover, onSelect, onClear }) {
    this.stage = stage;
  ...
  step(dir, e) {
    ...
    this.select(ids[(at + dir + ids.length) % ids.length]);
  }
  ...
  renderPanel(frag, index) {
    ...
    this.panel.append(head, data, body);
